...
clear all; 
close all; 
load noise.mat; % Load specific noise data (if necessary)
...
% Create a table to list all delays corresponding to DOA values
lag_range = -11:11; 
...
for i = 1:length(lag_range)
    ...
    DOA_table(i, :) = [Lag, rad2deg(theta_value)]; % Convert from radians to degrees
end
...
for i = 1:length(SNR_dB)
    ...
    correct_num = 0; % Initialize the count of correct DOA estimations to 0
    for j = 1:100
        % Randomly select a DOA
        true_DOA_index = randi(23);
        true_DOA = theta(true_DOA_index);
        ...
        DOA_estimate = asin(estimated_lag * c / (N * d)); % Calculate the estimated DOA
        
        % Calculate the error and update the count of correct estimations
